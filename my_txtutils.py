...
def sample_from_probabilities(probabilities, topn=ALPHASIZE, temp=1.):
...
def print_learning_learned_comparison(X, Y, losses, bookranges, batch_loss, batch_accuracy, epoch_size, index, epoch, lr):
    ...
    stats = "{: <28} batch loss: {:.5f}, batch accuracy: {:.5f}, learning rate: {:.5f}".format(batch_string, batch_loss, batch_accuracy, lr)
    print()
...
def read_data_files(directory, validation=True):
    ...
    for file in filelist:
        ...
        start = len(codetext)
        codetext.extend(encode_text(text.read()))
        end = len(codetext)
        fileranges.append({"start": start, "end": end, "name": file.rsplit("/", 1)[-1]})
        text.close()
        print(end)
        if end > 1024*1024*128:
            break
    ...
    validation_len = 0
    nb_books1 = 0
    for file in reversed(fileranges):
